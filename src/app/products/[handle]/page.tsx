...
function ProductPageClient({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const { data, loading, error } = useQuery(GET_PRODUCT_BY_HANDLE, {
    ...
    errorPolicy: 'all',
    skip: !decodedHandle // handleが取得されるまでクエリを実行しない
  })

  console.log('🔍 Product detail page - original handle:', handle)
  console.log('🔍 Product detail page - decoded handle:', decodedHandle)
  console.log('🔍 Product detail page - loading:', loading)
